    ...
    public IActionResult Editar(Usuario usuario, bool eliminarAvatar)
    {
        try
        {
            ...
            else if (eliminarAvatar)
            {
                ...
                var usuarioExistente = new RepositorioUsuario().GetUsuarioById(usuario.Id);
                if (usuarioExistente != null && !string.IsNullOrEmpty(usuarioExistente.AvatarUrl))
                {
                    // Ruta completa del avatar a eliminar
